...
function add_slider_scripts() {
  ...
  if ( is_home() ) {
    ...
    $siteurl = site_url();
    $homeurl = home_url();
    $slickstyledir = plugins_url( '', __FILE__ );
    $slickurl = str_replace($siteurl, $homeurl, $slickstyledir);

    //wp_enqueue_style( 'zalo-slider-slick', plugins_url( 'slick.css', __FILE__ ), array(), $pluginversion, false );
    wp_enqueue_style( 'zalo-slider-slick', $slickurl . '/slick.css', array(), $pluginversion, false );
